...
class UserViewTests(TestCase):
    ...
    def test_user_management_view_requires_login(self):
        """Test that user management view requires login"""
        response = self.client.get(reverse('user-management'))
        self.assertEqual(response.status_code, 302)  # Redirect to login
    
    def test_user_management_view_requires_permission(self):
        """Test that user management view requires admin permission"""
        self.client.login(username='testuser', password='testpass123')
        response = self.client.get(reverse('user-management'))
        self.assertEqual(response.status_code, 403)  # Forbidden
    
    def test_user_management_view_admin_access(self):
        """Test user management view for admin user"""
        self.client.login(username='admin', password='adminpass123')
        response = self.client.get(reverse('user-management'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'user/management.html')
        self.assertContains(response, 'testuser')
    ...
    def test_user_edit_approval_marks_email_verified_for_allauth(self):
        """Approving via user management must verify EmailAddress so login works."""
        from allauth.account.models import EmailAddress

        pending = self.User.objects.create_user(
            username='pending1',
        ...
        EmailAddress.objects.create(
            user=pending, email='p1@example.com', primary=True, verified=False
        )
        self.client.login(username='admin', password='adminpass123')
        form_data = {
            'username': 'pending1',
        ...
        response = self.client.post(
            reverse('user-edit', kwargs={'user_id': pending.pk}), form_data
        )
        self.assertEqual(response.status_code, 302)
        ea = EmailAddress.objects.get(user=pending, email__iexact='p1@example.com')
        self.assertTrue(ea.verified)
        self.assertTrue(ea.primary)

    def test_admin_delete_other_user_post(self):
        """Superuser can delete another user via admin delete view."""
        self.client.login(username='admin', password='adminpass123')
        victim = self.User.objects.create_user(
            'delete_me', 'del@example.com', 'pass-12345'
        )
        url = reverse('user-admin-delete', kwargs={'user_id': victim.pk})
        response = self.client.post(url)
        self.assertEqual(response.status_code, 302)
        self.assertFalse(self.User.objects.filter(pk=victim.pk).exists())

    def test_admin_delete_self_forbidden(self):
        self.client.login(username='admin', password='adminpass123')
        url = reverse('user-admin-delete', kwargs={'user_id': self.admin_user.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 403)

    def test_admin_delete_blocked_by_protected_fk_redirects(self):
        """When delete raises ProtectedError, show message and return to edit."""
        self.client.login(username='admin', password='adminpass123')
        victim = self.User.objects.create_user('prot_u', 'prot@example.com', 'x')
        url = reverse('user-admin-delete', kwargs={'user_id': victim.pk})

        def _raise_protected(self, *args, **kwargs):
            raise ProtectedError('blocked', set())

        with patch.object(self.User, 'delete', _raise_protected):
            response = self.client.post(url)
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response.url,
            reverse('user-edit', kwargs={'user_id': victim.pk}),
        )
        self.assertTrue(self.User.objects.filter(pk=victim.pk).exists())
...
class UserProfileViewSmokeTests(TestCase):
    """Profile URL tests using projects/labeling (no legacy datasets app)."""

    def setUp(self):
        from test_support.factories import create_project, create_user, create_labeling_basics

        self.user = create_user("pv1", "pv1@t.local", "p", is_approved=True)
        self.other = create_user("pv2", "pv2@t.local", "p", is_approved=True)
        self.admin = get_user_model().objects.create_superuser("pva", "pva@t.local", "p")
        self.project = create_project(self.user, title="P Smoke", access_level="private")
        self.schema, self.dataset = create_labeling_basics(self.project)

    def test_own_profile_200(self):
        self.client.login(username="pv1", password="p")
        r = self.client.get(reverse("user-profile"))
        self.assertEqual(r.status_code, 200)
        self.assertTemplateUsed(r, "user/profile.html")

    def test_detail_other_403(self):
        self.client.login(username="pv1", password="p")
        r = self.client.get(
            reverse("user-profile-detail", kwargs={"user_id": self.other.pk})
        )
        self.assertEqual(r.status_code, 403)

    def test_detail_200_superuser(self):
        self.client.login(username="pva", password="p")
        r = self.client.get(
            reverse("user-profile-detail", kwargs={"user_id": self.user.pk})
        )
        self.assertEqual(r.status_code, 200)
        self.assertTemplateUsed(r, "user/profile.html")
    ...
    def test_resolve_profile_url(self):
        ...
        self.assertEqual(resolve("/user/profile/").func.view_class, UserProfileView)
