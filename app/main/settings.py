...
# GeoDjango: optional native lib paths (e.g. macOS Homebrew; Docker/CI often find libs without this)
...
ALLOWED_HOSTS = os.environ.get('ALLOWED_HOSTS', 'localhost,127.0.0.1,0.0.0.0,isrlabel.dataplexity.eu').split(',')

# CSRF settings
CSRF_TRUSTED_ORIGINS = os.environ.get('CSRF_TRUSTED_ORIGINS', 'https://isrlabel.dataplexity.eu,http://localhost:8000,http://127.0.0.1:8000').split(',')
...
INSTALLED_APPS = [
    ...
    'django.contrib.gis',
    # Custom apps must come before third-party apps that depend on AUTH_USER_MODEL
    'user',
    'pages',
    'projects',
    'labeling',
...
# Database configuration (PostGIS / GeoDjango)
# https://docs.djangoproject.com/en/stable/ref/contrib/gis/
def _configure_postgis(config):
    """Use PostGIS backend for all environments."""
    config = dict(config)
    config['ENGINE'] = 'django.contrib.gis.db.backends.postgis'
    return config


# Use DATABASE_URL if available (e.g. CI), otherwise individual env vars
if 'DATABASE_URL' in os.environ:
    ...
    DATABASES = {
        'default': _configure_postgis(db_config)
    }
else:
    DATABASES = {
        'default': _configure_postgis({
            'NAME': os.environ.get('POSTGRES_DB', 'isrlabel'),
...
THUMBNAIL_ALIASES = {
    '': {
        'label_thumb': {'size': (320, 240), 'crop': 'smart'},
    },
}
